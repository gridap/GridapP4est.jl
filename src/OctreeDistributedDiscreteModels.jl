...
function update_face_to_entity_with_ghost_data!(face_to_entity,
    ...
    face_to_entity_pvec = PVector(face_to_entity, trian_cell_gids) do face_to_entity, trian_cell_gids
        ...
        nfaces  = length(values)
        face_lids = collect(1:nfaces)
        face_lids
    end
    ...
    if covers_all_faces
        ...
        if (Dcm==3)
            map(topology_new,cell_faces_new) do topology,cell_edges
              topology.n_m_to_nface_to_mfaces[Dcm+1,Dcm-1] = cell_edges[Dcm-1]
              topology.n_m_to_nface_to_mfaces[Dcm-1,Dcm+1] = Gridap.Geometry.generate_cells_around(cell_edges[Dcm-1])
            end
        end

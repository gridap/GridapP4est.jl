...
function generate_poisson_analytical_functions(order)
  u(x) = x[1]^order
  f(x) = -Δ(u)(x)
  u, f
end

function generate_maxwell_analytical_functions(order)
  u(x) = VectorValue(x[1]x[2]^order, x[2]x[1]^order)
  f(x) = ∇(∇ ⋅ u)(x) - Δ(u)(x) + u(x)
  u, f
end
...
function run(distribute)
  ...
  for order in (2, 4, 8)
    degree = 2 * order + 1
    I(x) = x[1]^order
    u, f = generate_poisson_analytical_functions(order)
  ...
  # Maxwell tests
  ### TO-DO: on 31 May 2024, order 7 is not working
  for order in (1, 3)
    u, f = generate_maxwell_analytical_functions(order)
    space_kwargs = Dict(:conformity => :Hcurl, :dirichlet_tags => "boundary")
    reffe = ReferenceFE(nedelec, Float64, order)
    VH = TestFESpace(modelH, reffe; space_kwargs...)
    UH = TrialFESpace(VH, u)
    Vh, modelh = Gridap.LinearizedFESpace(modelH, reffe; space_kwargs...)
    dΩH, dΩh = Measure(Triangulation(modelH), 2order + 1), Measure(Triangulation(modelh), order + 1)
    a(u, v, dΩ) = ∫((∇ × u) ⋅ (∇ × v))dΩ + ∫(u ⋅ v)dΩ
    l(v, dΩ) = ∫(f ⋅ v)dΩ
    uH = solve(AffineFEOperator((u, v) -> a(u, v, dΩH), v -> l(v, dΩH), UH, VH))
    uh = solve(AffineFEOperator((u, v) -> a(u, v, dΩh), v -> l(v, dΩh), UH, Vh))
    eH, eh = uH - u, uh - u
    ∇xeH, ∇xeh = ∇ × eH, ∇ × eh
    @test sqrt(∑(∫(eH ⋅ eH + ∇xeH ⋅ ∇xeH)dΩH)) < 1e-7
    @test sqrt(∑(∫(eh ⋅ eh + ∇xeh ⋅ ∇xeh)dΩH)) < 1e-7
  end
end
end
